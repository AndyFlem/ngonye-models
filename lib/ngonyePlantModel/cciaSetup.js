...
const cciaModels = modelNames.map(modelName=>{
  ...
  const FDC = [...Array(101).keys()].map(v=> {
    const ret = {
      exceedance: v/100,
      flow: d3.quantile(model.daily, 1-(v/100), d=>d.flow)
    }
    return ret
  })
  model.daily.forEach((v,i)=> {
...
fs.writeFileSync(folder + 'ngonyePlantModels/ccia/cciaModels_' + hydrologySet + '.csv', d3.csvFormatRows(modelNames.map(n=>[n])))
