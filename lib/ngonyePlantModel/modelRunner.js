...
models.forEach(model=>{
  ...
  if (runModel) {
    ...
    let daily = d3.csvParse(fs.readFileSync(folder + '/syntheticFlowSeries/' + params.hydrologySet + '/processed/daily.csv', 'utf-8'), d3.autoType).map(v=> {
      ...
      if (v.datetime.hour !=0) {
        //throw new Error(`Daily flow data must be at midnight, but found ${v.datetime.toISO()}`)
        v.datetime = v.datetime.startOf('day') // If not at midnight then set to midnight 
      }
      return v
    })  
    ...
    console.log(`Calculated stats for model ${modelRef} in ${DateTime.now().diff(start).as('milliseconds')} ms.`)
    ...
    // Save daily as json
    fs.writeFileSync(folder + 'ngonyePlantModels/models/' + params.modelRef + '/' + params.modelRef + '_pe_daily.json', JSON.stringify(daily, null, 2))
    ...
    fs.writeFileSync(folder + 'ngonyePlantModels/models/' + params.modelRef + '/' + params.modelRef + '_pe_weeklyslim.csv', d3.csvFormat(stats.weeklySlim))
    ...
    fs.writeFileSync(folder + 'ngonyePlantModels/models/' + params.modelRef + '/' + params.modelRef + '_pe_fdc.csv', d3.csvFormat(stats.fdc)) 
    fs.writeFileSync(folder + 'ngonyePlantModels/models/' + params.modelRef + '/' + params.modelRef + '_pe_durationvariability.csv', d3.csvFormat(stats.durationVariability)) 
  } else {
