...
export default function statistics(parameters, dys, fast) {
  ...
  const fdc = d3.rollups(dys, v => {
    return {
      ...
      units: Math.ceil(d3.mean(v, d=>d.generation.calc2.units)),
  ...
  function aggregateYearly(dys, fKey) {
    return d3.rollups(dys, v=>{
      let ret = {
        ...
        grossHeadMin: toP(d3.min(v, d => d.levels.grossHead), 3),
        grossHeadMax: toP(d3.max(v, d => d.levels.grossHead), 3),
        netHeadMin: toP(d3.min(v, d => d.generation.calc2.netHead), 3),
        netHeadMax: toP(d3.max(v, d => d.generation.calc2.netHead), 3),
        headlossMax: toP(d3.max(v, d => d.generation.calc2.headlossTotal), 3),
        headlossMin: toP(d3.min(v, d => d.generation.calc2.headlossTotal), 3),
        riverVolume: toP(d3.sum(v, d => d.flow) * 24 * 60 * 60 / 1000 / 1000 / 1000, 3), // km3
        eFlowVolume: toP(d3.sum(v, d => d.flows.eFlows.total) * 24 * 60 * 60 / 1000 / 1000 / 1000, 3), // km3
        canalVolume: toP(d3.sum(v, d => d.flows.canal) * 24 * 60 * 60 / 1000 / 1000 / 1000, 3), // km3
        eFlowProportionMean: toP(d3.mean(v, d => d.flows.eFlowProportion), 3),
  ...
  const durationVariability = (()=>{
    let durations = [5, 10, 15, 18, 20, 25, 50, 100]

    let stDev = d3.deviation(yearly,v=>v.energy)
    let mean = d3.mean(yearly,v=>v.energy)
    const exceedances = [...Array(21).keys()].map(v=> v/20).slice(1,-1)
    ...
    return durations.map(d=>{
      ...
      exceedances.forEach(ex=>{
        ret[ex] = normalinv(1-(ex), mean, stDev/Math.sqrt(d))
      })
      return ret
    })
  ...
  const bins=20
  const annualExceedances = [...Array(bins+1).keys()].map(v=> {
    return {
      exceedance: v/bins,
      energy: toP(d3.quantile(yearly, 1-(v/bins), d=>d.energy), 4),
      riverVolume: toP(d3.quantile(yearly, 1-(v/bins), d=>d.riverVolume), 3),
      capFactor: toP(d3.quantile(yearly, 1-(v/bins), d=>d.capFactor), 3),
      eFlowProportion: toP(d3.quantile(yearly, 1-(v/bins), d=>d.eFlowProportion), 3),
      specificEnergy: toP(d3.quantile(yearly, 1-(v/bins), d=>d.specificEnergy), 3),
      shutoffDaysProportion: toP(d3.quantile(yearly, 1-(v/bins), d=>d.shutoffDaysProportion), 3)
    }
  })
  ...
  yearly.forEach(v=>{
    v.energyExceedance = interpolate(annualExceedances, 'energy', 'exceedance', v.energy)
  })

  const statistics = {
    yearType: waterYearBasis ? 'Water' : 'Calendar',
    riverFlowMeanMean: toP(d3.mean(waterYearBasis ? yearly : calendarYearly, d => d.flowMean), 3),
    riverVolumeMean: toP(d3.mean(waterYearBasis ? yearly : calendarYearly, d => d.riverVolume), 3),
    ...
    powerMax: toP(d3.max(waterYearBasis ? yearly : calendarYearly, d => d.powerMax), 3),
  }
  statistics.Skew = (statistics.energyAnnualP50 - statistics.energyAnnualMean)/statistics.energyAnnualMean
  

  yearly.forEach(v=>{
    v.energyPctVariation = (v.energy - statistics.energyAnnualMean)/ statistics.energyAnnualMean
    v.riverVolumePctVariation = (v.riverVolume - statistics.riverVolumeMean)/ statistics.riverVolumeMean
    v.riverFlowPctVariation = (v.flowMean - statistics.riverFlowMeanMean)/ statistics.riverFlowMeanMean
  })



  return {fdc, daily,weekly, weeklySlim, monthly, yearly, calendarYearly, calMonthly, calMonthlyEnergyExceedances, dailyExceedances, monthlyExceedances, annualExceedances, statistics, durationVariability}
}
